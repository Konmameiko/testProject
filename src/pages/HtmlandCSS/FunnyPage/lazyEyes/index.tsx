...
const LazyEye: FC<IProps> = () => {
	...
	useEffect(() => {
		if (eyeState === 0) {
			eyeRef.current && (eyeRef.current.style.transform = `rotateY(0deg) rotateX(0deg)`);
			ballref.current && (ballref.current.style.transform = `translate(0px, 0px)`);
		}
		if (eyeState === 2) {
			timer = setTimeout(() => {
				setEyeState(3);
	...
	const handleMouseMove = (e: any) => {
		...
		if (mouseCoords[0] < -tmpX || mouseCoords[0] > tmpX || mouseCoords[1] > tmpY || mouseCoords[1] < -tmpY) {
			eyeRef.current && (eyeRef.current.style.transform = `rotateY(0deg) rotateX(0deg)`);
			ballref.current && (ballref.current.style.transform = `translate(0px, 0px)`);
		} else {
			// 旋转角度
			...
			eyeRef.current && (eyeRef.current.style.transform = `rotateY(${eyeYDeg}deg) rotateX(${eyeXDeg}deg)`);
			ballref.current && (ballref.current.style.transform = `translate(${eyeYDeg / 1.5}px, ${-eyeXDeg / 1.5}px)`);
		}
	};
	...
	return (
		...
		>
			...
			<div ref={eyeRef} className={`${styles.eyeSocket} ${styles[stateArr[eyeState]]}`}>
				<div ref={ballref} className={styles.eyeBall}></div>
			</div>
			{/* SVG滤镜 */}
			<div className={styles.filter} style={{ opacity: `${eyeState === 2 ? 1 : 0}` }}>
